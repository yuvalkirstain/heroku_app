...
num_urls = len(app.backend_urls)
num_urls = 100
MAX_SIZE_IN_QUEUE = num_urls * 1
MAX_SIZE_CONCURRENT = num_urls // 2
...
BLOCKED_IPS = ["159.138.50.118", "42.2.119.97", "5.28.184.13", "190.167.37.23", "62.102.148.166", "89.209.53.165",
               "217.84.145.197", "88.224.35.22"]
...
async def generate_images_via_api(prompt, negative_prompt, user_id, engine_id):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if "beta" in engine_id:
            prompt = "$IPCinline:{" + "\"preset\": \"xlfixer\", " + "\"sdxl_ascore\"" + f":[{asscore},2.5]" + "}$ " + prompt
        data = {}
        while not has_generated:
            try:
